...
{
  public:
    notify_t (const user_id id_arg, pt::ptime notify_time_arg, event_ptr notify_event_arg);
    ...
    const event_ptr &get_notifying_event () const {return notify_event;}
...
class notifies_t
{
  public:
    ...
    int add_notify (user_id &id, pt::ptime notify_time, event_ptr notify_event);
    void clear_user_noifies (user_id &id);
    int remove_user_one_notify (user_id &id, pt::ptime notify_time, event_ptr notify_event);
    void remove_user_event_notifies (user_id &id, event_ptr notify_event);
